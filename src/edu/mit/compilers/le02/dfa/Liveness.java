 ...
 * 
 */
public class Liveness extends BasicBlockVisitor implements
		Lattice<BitSet, BasicBlockNode> {
	...
	public class BlockItem extends GenKillItem {
		...
		public BlockItem(Liveness parent, BasicBlockNode node,
				List<BasicStatement> blockDefs) {
			this.parent = parent;
			this.node = node;
		...
		public Collection<WorklistItem<BitSet>> predecessors() {
			ArrayList<WorklistItem<BitSet>> ret = new ArrayList<WorklistItem<BitSet>>();
	...
	private VariableLocation getDefinitionTarget(OpStatement def) {
		...
		default:
			ErrorReporting.reportErrorCompat(new Exception(
					"Tried to get target " + "of a non definition!"));
			return null;
		}
	}
	...
	private List<VariableLocation> getDefinitionUses(OpStatement def) {
		...
		case NOT:
			if (def.getArg1() instanceof VariableArgument) {
				ret.add(((VariableArgument) def.getArg1()).getDesc()
						.getLocation());
			}
			break;
		case ADD:
		...
		case MODULO:
			...
			if (def.getArg2() instanceof VariableArgument) {
				ret.add(((VariableArgument) def.getArg2()).getDesc()
						.getLocation());
			}
			break;
		default:
