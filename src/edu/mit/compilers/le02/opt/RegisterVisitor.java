    ...
    for (BasicStatement stmt : node.getStatements()) {
      ...
      if (stmt.getType() == BasicStatementType.CALL) {
        ...
        if (web != null) {
          ...
          ASTNode startNode = startOfMethod.getNode();
          if (reg == null) {
            // Move the variable OUT of its register into a temporary
            int tmpOffset =
              startNode.getSymbolTable().getNonconflictingOffset();
            ...
            if (oldReg != null) {
              ...
              BasicStatement bs = new OpStatement(startNode,
                  AsmOp.MOVE,
            ...
            if (desc.getLocation().getLocationType() == LocationType.REGISTER) {
              if (argReassign == null) {
                argReassign = new ArgReassignStatement(startNode);
